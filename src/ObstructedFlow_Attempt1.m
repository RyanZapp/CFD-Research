...
% new stuff
nx_half = ceil(0.5*dxi*Lx);
ny_half = ceil(0.5*dyi*Ly);
xHalf_1 = nx_half + ceil(nx/9);
xHalf_2 = nx_half - ceil(nx/9);
yHalf_1 = ny_half + ceil(ny/9);
yHalf_2 = ny_half - ceil(ny/9);
%B = ones(nx,ny);
%B(xHalf_2:xHalf_1,yHalf_2:yHalf_1) = 0;

Bu = ones(nx+1,ny+2);
Bu(xHalf_2:(xHalf_1+1),(yHalf_2+1):(yHalf_1+1)) = 0;

Bv = ones(nx+2,ny+1);
Bv((xHalf_2+1):(xHalf_1+1),yHalf_2:(yHalf_1+1)) = 0;
...
L = NNND_pressure_matrix(nx,ny,dxi2,dyi2);
...
for ii = 1:nt
    ...
    u(1,:) = f;                       % x-velocity left B.C.
    u(end,:) = u(end-1,:);                     % x-velocity right B.C.
    ...
    u = u.*Bu; % imposes flow obstruction B.C. for u
    v = v.*Bv; % imposes flow obstruction B.C. for v
    % Repeat for the us and vs matrices
    % multiply the u matrix by the b matrix
    % set the inlet to a parabolic condition
    % set the outlet to a neumann condition
    % set pressure on the RHS to 1 (actually, that is going to mess up the
    % laplacian, so maybe just try to specify the inlet and outlet velocity
    %
    
    us(:,1) = 2*ubcb - us(:,2);         % x-velocity bottom B.C.
    us(:,end) = 2*ubct - us(:,end-1);   % x-velocity top B.C.
    us(1,:) = f;                       % x-velocity left B.C.
    us(end,:) = us(end-1,:);                     % x-velocity right B.C.
    ...
    us = us.*Bu; % imposes flow obstruction B.C. for u
    vs = vs.*Bv; % imposes flow obstruction B.C. for v
