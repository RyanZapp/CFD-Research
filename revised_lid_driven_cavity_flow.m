...
% Number of steps in x and y axis
nx = n;
ny = n;

% Dimensions of container cross-section
Lx = 1;
Ly = 1;
...
% Inversian of the Laplacian (Pressure Matrix)
L(1,:) = 0;
L(1,1) = 1;
...
%% "Predictor Step" - Spatial Data
    ...
    for j = 2:(ny+1)
        for i = 2:nx
            ...
            us(i,j) = u(i,j) + dt*(nu*(Lux(i-1,j-1) + Luy(i-1,j-1)) - (Nux(i-1,j-1) + Nuy(i-1,j-1))) ;
    ...
    for j = 2:ny
        for i = 2:(nx+1)
            ...
            % Convective Terms
            Nvx(i-1,j-1) = v(i,j)*(vce(i-1,j) - vce(i-1,j-1))*dxi; 
            Nvy(i-1,j-1) = u_here*(vco(i,j-1) - vco(i-1,j-1))*dyi;
            
            % Momemntum Conservation Finite Difference v-direction (EQ. 12 OWKES)
            vs(i,j) = v(i,j) + dt*(nu*(Lvx(i-1,j-1) + Lvy(i-1,j-1)) - (Nvx(i-1,j-1) + Nvy(i-1,j-1))) ;
...
% START COMMENT HERE IF NOT USING
